...
def run_analysis():
    ...
    if analyzer:
        with st.sidebar:
            ...
            # сделал так замдренно, потому что вначале был план сделать синхронный и асинхронный метод, но потом задумался (стоит чаще делать)
            # и понял что при одном вызове разницы не будет вообще никакой, поэтому решил оставить синхронный
            # если придумать как то и масштабировать, много запросов что б было, то конечно асинхронный круче, а с один запросом и нечего думать)
            method_map = {"Синхронный": "sync"}
            st.session_state["api_method"] = method_map.get("Синхронный", "sync")
